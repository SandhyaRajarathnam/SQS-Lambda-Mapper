...
resource "aws_s3_bucket" "lambda-terraform" {
	bucket="deploymentbucket-000"
	acl="private"
	tags= {
		Name="lambda-terraform-bucket-0"
	}
...
resource "aws_lambda_event_source_mapping" "function" {
	event_source_arn="arn:aws:sqs:eu-west-1:<uid>:terraform-sqs-queue.fifo"
	function_name="arn:aws:lambda:eu-west-1:<uid>:function:function"
	batch_size=5
}
